...
namespace Cholopol.TIS.MVVM.ViewModels
{
    public class TetrisGridVM : TetrisItemContainerVM
    {
        ...
        //private void CleanAllReferencesOfItem(TetrisItemVM item)
        //{
        //    if (item == null || TetrisItemOccupiedCells == null) return;
        //    for (int x = 0; x < GridSizeWidth; x++)
        //    {
        //        for (int y = 0; y < GridSizeHeight; y++)
        //        {
        //            if (TetrisItemOccupiedCells[x, y] == item)
        //                TetrisItemOccupiedCells[x, y] = null;
        //        }
        //    }
        //}
        ...
        public bool CanQuickExchange(TetrisItemGhostVM ghost, Vector2Int targetPos)
        {
            ...
            if (!BoundryCheck(targetPos.x, targetPos.y, ghost.Width, ghost.Height)) return false;
            ...
            {
                if (!PositionCheck(cell.x, cell.y)) return false;
        ...
        public bool TryQuickExchange(TetrisItemGhostVM ghost, Vector2Int targetPos)
        {
            ...
            fullyCovered.Sort((a, b) => InventoryManager.Instance.GetTetrisCoordinateSet(b.ItemDetails.tetrisPieceShape).Count.CompareTo(InventoryManager.Instance.GetTetrisCoordinateSet(a.ItemDetails.tetrisPieceShape).Count));
            ...
            for (int i = 0; i < fullyCovered.Count; i++)
            {
                ...
                var mapping = CalculateGridMapping(it, ghost.SelectedItem, targetPos);
        ...
        {
            var currentAbsToShape = new Dictionary<Vector2Int, Vector2Int>();
            var shape = item.TetrisCoordinateSet;
            for (int s = 0; s < shape.Count; s++)
            {
                var sp = shape[s];
                int cx = item.LocalGridCoordinate.x + sp.x + item.RotationOffset.x;
                int cy = item.LocalGridCoordinate.y + sp.y + item.RotationOffset.y;
                currentAbsToShape[new Vector2Int(cx, cy)] = sp;
            }
            ...
            {
                Vector2Int currentCell = kv.Key;
                Vector2Int originalCell = kv.Value;
                if (currentAbsToShape.TryGetValue(currentCell, out Vector2Int sp))
                {
                    for (int i = 0; i < dirList.Count; i++)
                    {
                        ...
                        bool canPlace = true;
                        var required = new List<Vector2Int>();
                        for (int k = 0; k < rotatedPoints.Count; k++)
                        {
                            var p = rotatedPoints[k];
                            int x = originalCell.x + p.x + rotationOffset.x - sp.x;
                            int y = originalCell.y + p.y + rotationOffset.y - sp.y;
                            if (x < 0 || x >= originGrid.GridSizeWidth || y < 0 || y >= originGrid.GridSizeHeight) { canPlace = false; break; }
                            var abs = new Vector2Int(x, y);
                            if (!originalAreaSet.Contains(abs)) { canPlace = false; break; }
                            if (originGrid.TetrisItemOccupiedCells[x, y] != null) { canPlace = false; break; }
                            required.Add(abs);
                        }
                        if (canPlace)
                        {
                            ...
                            int anchorX = originalCell.x - sp.x + rotationOffset.x;
                            int anchorY = originalCell.y - sp.y + rotationOffset.y;
                            if (originGrid.IsAreaVacantForItem(item, anchorX, anchorY))
                            {
                                originGrid.PlaceTetrisItem(item, anchorX, anchorY);
                                return true;
                            }
                        }
                    }
